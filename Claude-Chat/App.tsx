...
import { SlideCanvas, defaultTheme, SLIDE_WIDTH, SLIDE_HEIGHT, Theme } from "./components/SlideCanvas";
import { SlideConfig, SlideBlock, KPIBlock, ChartBlock, TableBlock, TextBlock } from "./types";
...
const exampleSlides: SlideConfig[] = [
  {
    ...
    blocks: [
      ...
      } as KPIBlock,
      {
        ...
        variant: "default",
        x: 910, y: 76, width: 290, height: 110,
      } as KPIBlock,
      {
        id: "chart1", type: "chart", chartType: "bar",
        title: "Umsatz nach Monat (€ T)",
        labels: ["Jan", "Feb", "Mär"],
        datasets: [
      ...
      } as ChartBlock,
      {
        ...
        x: 620, y: 206, width: 560, height: 460,
      } as ChartBlock,
    ],
  },
  {
    ...
    blocks: [
      ...
      } as TextBlock,
      {
        ...
        x: 40, y: 108, width: 1160, height: 560,
      } as TableBlock,
    ],
  },
  {
    ...
    blocks: [
      ...
      } as ChartBlock,
      {
        ...
        variant: "highlight",
        x: 800, y: 76, width: 400, height: 120,
      } as KPIBlock,
      {
        ...
        variant: "default",
        x: 800, y: 216, width: 400, height: 120,
      } as KPIBlock,
      {
...
function SlideThumbnail({
  slide, theme, active, index, onClick,
}: {
  ...
  return (
    ...
    >
      <div style={{
        ...
        borderRadius: "4px",
        overflow: "hidden",
        border: active ? `2px solid #E8A020` : `1.5px solid rgba(255,255,255,0.12)`,
      ...
      <span style={{
        fontSize: "10px", color: active ? "#E8A020" : "rgba(255,255,255,0.45)",
...
export default function App() {
      ...
      <div style={{
        ...
        <div style={{
          ...
          <button
            ...
            style={btnStyle}
          >Weiter →</button>
          <button onClick={() => setIsPresentMode(false)} style={{ ...btnStyle, color: "#E8A020" }}>
            ✕ Beenden
          </button>
        </div>
      </div>
  ...
  return (
    <div style={{
      ...
      background: "#141820", fontFamily: "'Barlow', sans-serif",
    }}>
      {/* Slide Panel (left) */}
      <div style={{
      ...
      <div style={{
        ...
        <div style={{
          ...
          <span style={{ color: "rgba(255,255,255,0.3)", fontSize: "12px" }}>
            Folie {currentIdx + 1} von {slides.length}
          </span>
          <button onClick={() => setIsPresentMode(true)} style={toolbarBtn}>
            ▶ Präsentieren
          </button>
          <button onClick={handleExportPDF} style={{ ...toolbarBtn, background: "#E8A020", color: "#0F1318", borderColor: "#E8A020" }}>
            ↓ PDF
          </button>
        </div>
        ...
        <div style={{
          ...
          <button onClick={() => setCurrentIdx(Math.max(0, currentIdx - 1))} disabled={currentIdx === 0} style={navBtn}>
            ← Zurück
          </button>
          {slides.map((_, i) => (
            <div
              ...
              style={{
                ...
                height: "6px",
                borderRadius: "3px",
                background: i === currentIdx ? "#E8A020" : "rgba(255,255,255,0.2)",
                cursor: "pointer",
                transition: "all 0.2s",
              }}
            />
          ))}
          <button onClick={() => setCurrentIdx(Math.min(slides.length - 1, currentIdx + 1))} disabled={currentIdx === slides.length - 1} style={navBtn}>
            Weiter →
          </button>
        </div>
      </div>
